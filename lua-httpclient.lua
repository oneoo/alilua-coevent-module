...
if ngx and ngx.say then
	print = ngx.say
	tcp = ngx.socket.tcp
	base64encode = ngx.encode_base64
	base64decode = ngx.decode_base64
else
	tcp = cosocket.tcp
end
...
local function httprequest(url, params)
	...
	if params.timeout then
		sock:set_timeout(params.timeout)
	end
	...
	local is_post = false
	--multipart/form-data
	local boundary = '--'..base64encode(os.time()..math.random()):sub(1,16)
	...
	local request_headers = {params.method..' '..uri..' HTTP/1.1',
							...
							'Connection: '..(params.pool_size and 'keep-alive' or 'close'),
							'Accept: */*',
							}
	if zlib then
	...
	if user and pw then
		table.insert(request_headers, 'Authorization: Basic ' .. base64encode(user..':'..pw))
	end
